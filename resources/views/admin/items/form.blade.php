<div class="">
    ...
    <div class="p-4">
        <input type="text" name="name" id="name" value="{{ old('name', $item->name) }}" placeholder="Nombre del rubro" class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"/>
        @if($errors->has('name'))       
...
<div class="p-4">
    <button type="submit" class="hover:shadow-form rounded-md bg-sky-500/100 py-3 px-8 text-base font-semibold text-white outline-none">
        {{ $item->name ? __('Actualizar') : __('Agregar') }}
    </button>
    <a href="{{route('items.index')}}" class="hover:shadow-form rounded-md  py-3 px-8 text-base font-semibold text-black outline-none">
        Cancelar
     </a>
</div>